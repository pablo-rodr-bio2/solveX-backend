...
class QuoteController{
  ...
  async getToken(req: Request, res: Response): Promise<Response> {
    ...
    return jwt.sign({ user: user }, process.env.JWT_SECRET, (err: string, token: string) => {
      res.json({
        token: token
      })
    })
  ...
  async readAll(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(500).json({ msg: "Failed to read quotes", route: "/api/quotes" })
    }
  }
  ...
  async readById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(500).json({ msg: "Failed to read a quote by id", route: "/api/quote/:id" })
    }
  }
  ...
  async updateById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(201).json({ quote: updatedQuote})
    } catch (e) {
      return res.status(500).json({ msg: "Failed to update a quote", route: "/api/update/:id" })
    }
  }
  ...
  async deleteById (req: Request, res: Response) : Promise<Response> {
    try {
      ...
      return res.json({ msg: "Quote succesfully deleted", quote: deletedQuote })
    } catch (e) {
      return res.status(500).json({ msg: "Failed to update a quote", route: "/api/delete/:id" })
    }
  }
  ...
  async createQuote (req: Request, res: Response) : Promise<Response> {
    ...
    try {
      ...
      return res.status(201).json({ quote, msg: "You have created a new quote" })
    } catch (e) {
      return res.status(500).json({ msg: "Failed to create a new quote", route: "/api/new-quote" })
    }
  }
